...
The 0–9 roadmap is implemented. The current handoff is `1.0.0-rc.6`: a release candidate with path-accurate preview, game-frame-safe high-speed input, exact automatic HEX color entry, a source-color reproduction mode, clean line tracing, adaptive local smart-subject processing, face-feature-first ordering, and the compact rounded floating UI.
...
- `artifacts/GameDraw-1.0.0-rc.6-win-x64.zip`
- `artifacts/GameDraw-1.0.0-rc.6-win-x64.zip.sha256`

Use a new version when creating another build:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\Build-Release.ps1 -Version 1.0.0-rc.6
```
...
7. Select **Automatic color**. The UI automatically chooses **Source color reproduction**; confirm that every palette group focuses the calibrated HEX field, replaces it with `#RRGGBB`, commits it with Enter, and draws the matching colored runs.
