...
The current handoff is `1.0.0-rc.8`. It adds natural dark-mark centreline extraction, precision outline and ink-preserving alternatives, exact-color and pixel modes, four quality presets, game-sampled mouse interpolation, F5 global start, confirmed pen-up transitions, automatic Podiums HEX entry, and a clipped rounded execution panel.
...
- `artifacts/GameDraw-1.0.0-rc.8-win-x64.zip`
- `artifacts/GameDraw-1.0.0-rc.8-win-x64.zip.sha256`

Use a new version when creating another build:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\Build-Release.ps1 -Version 1.0.0-rc.8
```
