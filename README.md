...
현재 브랜치는 원본 색상 재현·자동 HEX 입력·검정 선화, 실행 경로와 동일한 미리보기, 다중 배경색을 구분하는 로컬 스마트 피사체 크롭, 인물 얼굴 특징 우선 순서, 게임 프레임 동기화 고속 실행, 둥근 컴팩트 플로팅 작업창과 배포 검증을 포함한 최종 릴리스 후보입니다.

기본 사용 순서는 다음과 같습니다.

1. `이미지 선택`으로 PNG/JPG/WEBP/BMP 파일을 불러옵니다.
2. `자동 채색` 또는 `검정 선화`, 실행 속도와 그리기 모드를 고릅니다. 필요하면 `스마트 피사체`로 배경 제거·피사체 크롭·인물 우선 순서를 켠 뒤 `이미지 분석`을 누릅니다.
...
검정 선화를 사람 손으로 정돈해 긋는 느낌으로 만들려면 `클린 펜 스트로크`를 선택하세요. 노이즈를 억제한 1픽셀 중심선을 갈림점에서도 가능한 한 곧게 이어 한 번의 연속 획으로 만들며, Podiums 브러시는 자동으로 최소 굵기에 맞춥니다. 분석 뒤 미리보기는 변환 픽셀이 아니라 실제 마우스가 실행할 경로를 표시합니다.
