...
4. 도구 위치가 아직 없을 때만 연필·지우개·채우기·굵기 최소/최대·HEX 입력란의 6개 위치를 차례로 가리키며 `F6`을 누릅니다.
5. `Podiums에 그리기 시작`을 누릅니다. GameDraw가 자동으로 숨고 Roblox를 활성화한 뒤 마우스 입력을 시작합니다.

실행 중 `F7`은 일시 정지/재개, `F8`은 마우스를 즉시 해제하고 중지합니다. 프로필 공유 기능은 상단 `고급 설정`에서 사용할 수 있습니다.

`게임 위에 띄우기`를 켜면 작업창이 화면 오른쪽에 맞춰지고 항상 위에 유지됩니다. 실행 중에는 GameDraw 본창을 자동으로 숨겨 Podiums 도구와 HEX 입력란을 가리지 않습니다. 자동 감지 결과가 드래그한 캔버스와 비슷하면 현재 위치를 보정하고, 크게 다르면 사용자가 지정한 영역을 우선해 엉뚱한 흰 영역으로 그리는 것을 막습니다.
