...
검정 선화를 사람 손으로 정돈해 긋는 느낌으로 만들려면 `클린 펜 스트로크`를 선택하세요. 선을 1픽셀 중심선으로 얇게 만든 뒤 갈림점 사이를 한 번의 연속 획으로 묶고 픽셀 지그재그를 줄입니다. `자동 추천`도 검정 선화에서는 이 방식을 사용합니다. 각 방식의 용도와 차이는 앱의 `사용법`에서 확인할 수 있습니다.
